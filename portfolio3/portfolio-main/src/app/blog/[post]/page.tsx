...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: `${post.title}`,
        metadataBase: new URL(`https://krishspatel.com/blog/${post.slug}`),
        description: post.description,
        publisher: post.author.name,
        keywords: post.tags,
        alternates: {
            canonical:
                post.canonicalLink || `https://krishspatel.com/blog/${post.slug}`,
        },
        openGraph: {
            images:
                urlFor(post.coverImage?.image).width(1200).height(630).url() ||
                fallbackImage,
            url: `https://krishspatel.com/blog/${post.slug}`,
            title: post.title,
            description: post.description,
            type: "article",
        ...
        twitter: {
            ...
            images:
                ...
                fallbackImage,
            creator: `@${post.author.githubURL.split("github.com/")[1]}`,
            site: `@${post.author.githubURL.split("github.com/")[1]}`,
...
export default async function Post({ params }: Props) {
    ...
    return (
        <main className="max-w-7xl mx-auto md:px-16 px-6">
            <header>
                <Slide
                    className="relative flex items-center gap-x-2 border-b dark:border-zinc-800 border-zinc-200 pb-8">
                    <Link
                        href="/blog"
            ...
            <article>
                ...
                >
                    <div
                        className="min-h-full lg:border-r border-r-0 dark:border-zinc-800 border-zinc-200 pt-10 pb-4 lg:pr-6 px-0">
                        <div
                            className="flex items-center flex-wrap gap-4 text-md mb-8 dark:text-zinc-400 text-zinc-600">
                    ...
                    <aside
                        className="flex flex-col lg:max-h-full h-max gap-y-8 sticky top-2 bottom-auto right-0 py-10 lg:px-6 px-0">
                            ...
                            <address className="flex items-center gap-x-3 mt-4 not-italic">
                                ...
                                <div rel="author">
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                    >
                                        {`@${post.author.githubURL.split("github.com/")[1]}`}
                                    </a>
                                </div>
                            </address>
                        </section>
